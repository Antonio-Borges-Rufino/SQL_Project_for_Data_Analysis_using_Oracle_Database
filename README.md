...
```
65. NA BASE DE DADOS QUE FOI TRABALHADA, EXISTIA APENAS UMA COLUNA COM O NOME COMPLETO, A MISSÃO FOI PARA SEPARAR EM DUAS COLUNAS, PARA ISSO USOU-SE AS FUNÇÕES SUBSTR E INSTR
```
-- SPLITANDO FULL NAME EM NOME E SOBRENOME SEPARADOS
SELECT c.full_name,
RTRIM(SUBSTR(c.full_name,1,INSTR(c.full_name,' '))),
LTRIM(SUBSTR(c.full_name,INSTR(c.full_name,' '),LENGTH(c.full_name)))
FROM customers c;
```
66. DEPOIS DISSO, AS TABELAS FORAM ATUALIZADA
```
-- AGORA BASTA APLICAR NO UPDATE NAS TABELAS
UPDATE customers c
SET c.first_name = RTRIM(SUBSTR(c.full_name,1,INSTR(c.full_name,' '))),
c.last_name = LTRIM(SUBSTR(c.full_name,INSTR(c.full_name,' '),LENGTH(c.full_name)));
commit;
```
67. A BASE DE DADOS POSSUIAM OUTROS PROBLEMAS QUE FORAM RESOLVIDOS, COM PADRONIZAÇÃO DE CARACTER USANDO SUBSTR E INSTR
```
-- OS EMAILS NAO POSSUEM O .COM NO FINAL
UPDATE customers c
SET c.email_address = c.email_address||'.com'
WHERE c.email_address NOT LIKE('%.com%');
commit;

-- OS NUMEROS DE CONTATO TAMBEM ESTÃO FORA DO PADRÃO
-- TIRAR TUDO QUE TIVER A PARTIR DO ','
UPDATE customers c
SET c.contact_number = SUBSTR(c.contact_number,1,INSTR(c.contact_number,',')-1)
WHERE c.contact_number LIKE('%,%');
commit;

-- OS NUMEROS DE TELEFONE PRECISAM CONTER 10 NUMEROS
-- QUALQUER COISA FORA ISSO DEVE VIRAR 9999999999
SELECT c.contact_number, LENGTH(c.contact_number)
FROM customers c
WHERE LENGTH(c.contact_number) < 10;
-- FAZER O UPDATE PARA OS NUMEROA MENORES QUE 10
UPDATE customers c
SET c.contact_number = 9999999999
WHERE LENGTH(c.contact_number)<10;
commit;
-- FAZER O UPDATE PARA OS NUMEROA MAIORES QUE 10
UPDATE customers c
SET c.contact_number = 9999999999
WHERE LENGTH(c.contact_number)>10;
commit;
```
68. OUTRO CASO DE USO DA MESMA BASE DE DADOS FOI A LEITURA DO TIPO BLOB, PARA ISSO, USOU-SE A FUNÇÃO DE LEITURA DE JSON
```
-- EXISTE DIVERSAS COLUNAS DENTRO DESSA COLUNA DO TIPO BLOBER, AQUI ESPECIFICAMENTE VOU LER COLOUR
SELECT PRODUCT_ID, 
PRODUCT_NAME, 
UNIT_PRICE, 
PRODUCT_DETAILS,
...
FROM PRODUCTS, JSON_TABLE(
    ...
    COLUMNS(
    colour_name varchar(50) path '$.colour',
    gender_type varchar(50) path '$.gender'
    )
);
```
69. COMO O FORMATO ERA JSON, PODERIAM EXISTIR COLUNAS DENTRO DE OUTRAS COLUNAS, ISSO ACONTECE NA COLUNA REVIEW, PARA ACESSAR A INFORMAÇÃO FAZ-SE
```
SELECT PRODUCT_ID,
PRODUCT_NAME, 
rating
...
```
70. OUTRO PROBLEMA DE FORMATAÇÃO E QUE EM JSON ALGUNS DADOS PODEM VIR COMO ARRAYS. PARA LE-LOS USA-SE FORMAT
```
-- DENTRO DE UMA COLUNA JSON, PODE TER OUTROS TIPOS DE FORMATO NAO LEGIVEIS, COMO É O CASO DE SIZE
-- SIZE ESTÁ NO FORMATO DE ARRAY, PARA ISSO PODEMOS LER USANDO FORMAT JSON
SELECT PRODUCT_ID, 
PRODUCT_NAME, 
colour_name,
gender_type,
...
FROM PRODUCTS, JSON_TABLE(
    ...
    colour_name varchar(50) path '$.colour',
    gender_type varchar(50) path '$.gender',
    tamanhos FORMAT JSON path '$.sizes'
    )
);
```
71. POR FIM, AGORA EXTRAIMOS AS INFORMAÇÕES CONTIDAS DENTRO DA COLUNA, SEPARANDO-A EM 2, UMA COM AS INFORMAÇÕES DE NIVEIS MAIS ALTOS, OU SEJA, ACESSADA DIRETAMENTE DENTRO DO ARQUIVO JSON
```
-- CRIANDO NOVAS TABELAS COM INFORMAÇÕES MAIS ENXUTAS
-- CRIANDO TABELA PARA RETIRAR AS INFOS DO BLOOB
CREATE TABLE PRODUCT_DESCRIPTION AS
SELECT PRODUCT_ID, 
PRODUCT_NAME, 
UNIT_PRICE,
COLOUR_NAME,
...
```
72. E OUTRA TABELA PARA INFORMAÇÕES QUE ESTÃO DENTRO DE OUTRAS TABELAS, EM NIVEIS MAIS BAIXOS. NESSE CASO, ISSO É IMPORTANTE PORQUE NÃO PODEMOS RETIRAR TODAS AS INFORMAÇÕES CONTIDAS AQUI POIS MUITAS DELAS DIVIDEM O MESMO ID, O QUE NÃO DARIA CERTO, PARA ISSO CRIAMOS OUTRA COLUNA E DEPOIS BASTA FAZER UM JOIN PARA PODER LER DE FORMA COMPLETA
```
-- CRIANDO A TABELA DE REVIEWS
CREATE TABLE PRODUCT_REV AS
SELECT PRODUCT_ID,
PRODUCT_NAME, 
rating,review
FROM PRODUCTS, JSON_TABLE(
    ...
    COLUMNS(
    rating NUMBER PATH '$.rating',
    review VARCHAR(200) PATH '$.review'
    )
);
```
